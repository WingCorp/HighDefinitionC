...
int list_consTwiceIsListOfLengthTwo()
{
    ...
    Dynamic consv1 = list_head(consd1);
    Dynamic consv1Val = coerce(consv1);
    printdi32(consv1Val);
...
int list_fromWorks()
{
    List* list = list_from(3, di32(1), di32(2), di32(3));
    int size = list_length(list);
    Dynamic head = list_head(list);
    return assertTrue("head is SOME", isSome(head)) && assertIntEquals(1, i32(coerce(head))) && assertIntEquals(size, 3);
}

int main()
{
    test_declareAndRun("list_empty() actually returns empty list", list_emptyListReturnsEmpty);
    test_declareAndRun("list_from works as intended", list_fromWorks);
